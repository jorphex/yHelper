  ...
  return (
    <>
      ...
      <TableWrap><table><thead><tr>
        ...
        <th aria-sort={sort.key === "chain" ? (sort.direction === "asc" ? "ascending" : "descending") : "none"}><button className="th-button" onClick={() => setSort(toggleSort(sort, "chain"))}>Chain {sortIndicator(sort, "chain")}</button></th>
        ...
        <th className="numeric" aria-sort={sort.key === "current" ? (sort.direction === "asc" ? "ascending" : "descending") : "none"}><button className="th-button" onClick={() => setSort(toggleSort(sort, "current"))}>Current {windowLabel} APY {sortIndicator(sort, "current")}</button></th>
        <th className="numeric" aria-sort={sort.key === "previous" ? (sort.direction === "asc" ? "ascending" : "descending") : "none"}><button className="th-button" onClick={() => setSort(toggleSort(sort, "previous"))}>Prior {windowLabel} APY {sortIndicator(sort, "previous")}</button></th>
        <th className="numeric" aria-sort={sort.key === "delta" ? (sort.direction === "asc" ? "ascending" : "descending") : "none"}><button className="th-button" onClick={() => setSort(toggleSort(sort, "delta"))}>Change {sortIndicator(sort, "delta")}</button></th>
      </tr></thead><tbody>
        {sortedRows.length === 0 ? <tr><td colSpan={6} className="muted">No {title.toLowerCase()} in this window.</td></tr> : visibleRows.map((row) => <tr key={`${title}:${row.chain_id}:${row.vault_address}`}><td><VaultLink chainId={row.chain_id} vaultAddress={row.vault_address} symbol={row.symbol} /></td><td><Link href={`/explore?chain=${row.chain_id}&universe=${universe}&market=${market}`}>{compactChainLabel(row.chain_id, compact)}</Link></td><td className="numeric">{formatUsd(row.tvl_usd)}</td><td className="numeric">{formatPct(row.realized_apy_window)}</td><td className="numeric">{formatPct(row.realized_apy_prev_window)}</td><td className={`numeric ${(row.delta_apy ?? 0) > 0 ? "text-positive" : "text-negative"}`}>{deltaArrow(row.delta_apy)} {formatPercentagePoints(row.delta_apy)}</td></tr>)}
