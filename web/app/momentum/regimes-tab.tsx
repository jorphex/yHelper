    ...
    <>
        ...
        <div className="card">
          <div className="filter-grid">
            <label>
              ...
              <select value={query.universe} onChange={(e) => updateQuery({ universe: e.target.value, min_tvl: null, min_points: null })} className="filter-control">
            ...
            <label>
              ...
              <select value={query.chain > 0 ? String(query.chain) : ""} onChange={(e) => updateQuery({ chain: e.target.value || null })} className="filter-control">
            ...
            <label>
              ...
              <input type="number" min={0} max={365} value={query.minPoints} onChange={(e) => updateQuery({ min_points: Number(e.target.value || 0) })} className="filter-control" />
            </label>
          </div>
        </div>
      </section>
      ...
      <section className="section section-md">
        <div className="card card-elevated">
          <h3 className="panel-title">Transition Analysis</h3>
          <div className="filter-grid">
            <label>
              <span className="filter-label">Movers Limit</span>
              <select value={query.limit} onChange={(e) => updateQuery({ limit: Number(e.target.value) })} className="filter-control">
            ...
            <label>
              ...
              <select value={query.transitionSplit} onChange={(e) => updateQuery({ transition_split: e.target.value })} className="filter-control">
            ...
            <label>
              ...
              <select value={query.transitionDays} onChange={(e) => updateQuery({ transition_days: e.target.value })} className="filter-control">
            ...
            <label>
              ...
              <input type="number" min={0} value={query.transitionMinCohortTvl} onChange={(e) => updateQuery({ transition_min_cohort_tvl: Number(e.target.value || 0) })} className="filter-control" />
            </label>
          </div>
        </div>
      </section>
        ...
        {regimeLoading ? (
          ...
          <div className="kpi-grid kpi-grid-6">
            ...
            <div className="kpi-card">
              ...
              <div className="kpi-value capitalize">{dominantRegime}</div>
              <div className="kpi-hint">Largest TVL share</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-label">Rising</div>
              <div className="kpi-value text-positive">{regimes.find((row) => row.regime === "rising")?.vaults ?? 0}</div>
            ...
            <div className="kpi-card">
              ...
              <div className="kpi-value text-negative">{regimes.find((row) => row.regime === "falling")?.vaults ?? 0}</div>
            ...
            <thead>
              <tr>
                <th aria-sort={summarySort.key === "regime" ? (summarySort.direction === "asc" ? "ascending" : "descending") : "none"}>
                  <button className="th-button" onClick={() => setSummarySort(toggleSort(summarySort, "regime"))}>
                    Regime <span>{sortIndicator(summarySort, "regime")}</span>
                  </button>
                </th>
                <th className="numeric" aria-sort={summarySort.key === "vaults" ? (summarySort.direction === "asc" ? "ascending" : "descending") : "none"}>
                  <button className="th-button" onClick={() => setSummarySort(toggleSort(summarySort, "vaults"))}>
                    Vaults <span>{sortIndicator(summarySort, "vaults")}</span>
                  </button>
                </th>
                <th className="numeric" aria-sort={summarySort.key === "tvl" ? (summarySort.direction === "asc" ? "ascending" : "descending") : "none"}>
                  <button className="th-button" onClick={() => setSummarySort(toggleSort(summarySort, "tvl"))}>
                    TVL <span>{sortIndicator(summarySort, "tvl")}</span>
                  </button>
                </th>
                <th className="numeric">TVL Share</th>
              </tr>
            </thead>
            <tbody>
              {regimeLoading ? (
                ...
                <tr key={row.regime}>
                  <td className="capitalize">{compactRegimeLabel(row.regime)}</td>
                  <td className="data-value numeric">{row.vaults}</td>
                  <td className="data-value numeric">{formatUsd(row.tvl_usd)}</td>
                  <td className="data-value numeric">{totalRegimeTvl > 0 ? formatPct((row.tvl_usd ?? 0) / totalRegimeTvl) : "n/a"}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <thead>
              <tr>
                <th aria-sort={regimeMoverSort.key === "vault" ? (regimeMoverSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                  <button className="th-button" onClick={() => setRegimeMoverSort(toggleSort(regimeMoverSort, "vault"))}>
                ...
                <th aria-sort={regimeMoverSort.key === "chain" ? (regimeMoverSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                  <button className="th-button" onClick={() => setRegimeMoverSort(toggleSort(regimeMoverSort, "chain"))}>
                ...
                {!isCompactViewport && (
                  ...
                  <button className="th-button" onClick={() => setRegimeMoverSort(toggleSort(regimeMoverSort, "token"))}>
                      Token <span>{sortIndicator(regimeMoverSort, "token")}</span>
                    </button>
                  </th>
                )}
                <th className="numeric" aria-sort={regimeMoverSort.key === "tvl" ? (regimeMoverSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                  <button className="th-button" onClick={() => setRegimeMoverSort(toggleSort(regimeMoverSort, "tvl"))}>
                    TVL <span>{sortIndicator(regimeMoverSort, "tvl")}</span>
                  </button>
                </th>
                <th className="numeric" aria-sort={regimeMoverSort.key === "apy" ? (regimeMoverSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                  <button className="th-button" onClick={() => setRegimeMoverSort(toggleSort(regimeMoverSort, "apy"))}>
                    Realized APY 30d <span>{sortIndicator(regimeMoverSort, "apy")}</span>
                  </button>
                </th>
                <th className="numeric" aria-sort={regimeMoverSort.key === "momentum" ? (regimeMoverSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                  <button className="th-button" onClick={() => setRegimeMoverSort(toggleSort(regimeMoverSort, "momentum"))}>
                    Momentum <span>{sortIndicator(regimeMoverSort, "momentum")}</span>
                  </button>
                </th>
                {!isCompactViewport && (
                  <th className="numeric" aria-sort={regimeMoverSort.key === "regime" ? (regimeMoverSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                    <button className="th-button" onClick={() => setRegimeMoverSort(toggleSort(regimeMoverSort, "regime"))}>
            ...
            <tbody>
              {regimeLoading ? (
                ...
                <tr key={row.vault_address}>
                  ...
                  {!isCompactViewport && <td className="data-value numeric" title={compactRegimeLabel(row.regime)}>{compactRegimeLabel(row.regime)}</td>}
                </tr>
              ))}
            </tbody>
          </table>
                ...
                <thead>
                  <tr>
                    <th aria-sort={splitSnapshotSort.key === "cohort" ? (splitSnapshotSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                      <button className="th-button" onClick={() => setSplitSnapshotSort(toggleSort(splitSnapshotSort, "cohort"))}>
                        Cohort <span>{sortIndicator(splitSnapshotSort, "cohort")}</span>
                      </button>
                    </th>
                    <th className="numeric" aria-sort={splitSnapshotSort.key === "churn" ? (splitSnapshotSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                      <button className="th-button" onClick={() => setSplitSnapshotSort(toggleSort(splitSnapshotSort, "churn"))}>
                        Churn % <span>{sortIndicator(splitSnapshotSort, "churn")}</span>
                      </button>
                    </th>
                    <th className="numeric" aria-sort={splitSnapshotSort.key === "churn_tvl" ? (splitSnapshotSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                      <button className="th-button" onClick={() => setSplitSnapshotSort(toggleSort(splitSnapshotSort, "churn_tvl"))}>
                        Churn TVL % <span>{sortIndicator(splitSnapshotSort, "churn_tvl")}</span>
                      </button>
                    </th>
                    <th className="numeric" aria-sort={splitSnapshotSort.key === "momentum" ? (splitSnapshotSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                      <button className="th-button" onClick={() => setSplitSnapshotSort(toggleSort(splitSnapshotSort, "momentum"))}>
                        Momentum <span>{sortIndicator(splitSnapshotSort, "momentum")}</span>
                      </button>
                    </th>
                    <th className="numeric" aria-sort={splitSnapshotSort.key === "tvl" ? (splitSnapshotSort.direction === "asc" ? "ascending" : "descending") : "none"}>
                      <button className="th-button" onClick={() => setSplitSnapshotSort(toggleSort(splitSnapshotSort, "tvl"))}>
                ...
                <tbody>
                  {splitSnapshotRows.map((row) => (
                    <tr key={`split-latest-${row.group_key}`}>
                      ...
                      <td className="data-value numeric">{formatPct(row.changed_ratio, 2)}</td>
                      <td className="data-value numeric">{formatPct(row.changed_tvl_ratio, 2)}</td>
                      <td className="data-value numeric">{formatPct(row.momentum_spread, 2)}</td>
                      <td className="data-value numeric">{formatUsd(row.tvl_total_usd)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
